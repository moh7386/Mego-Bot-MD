...
const handler = async (m, {conn, text, usedPrefix, command}) => {
  ...
  if (!text) throw `*يرجى كتابة ما تريد البحث عنه*\n\n*—◉ مثال:-*\n*◉ ${usedPrefix + command} كيف الحال؟*\n*◉ ${usedPrefix + command} احبك*`;
  try {
      ...
      try {
        conn.sendPresenceUpdate('composing', m.chat);
        const syms1 = `انا شات جي بي تي اعمل بمادارا بوت`;
