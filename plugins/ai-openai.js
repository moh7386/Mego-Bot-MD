...
const handler = async (m, {conn, text, usedPrefix, command}) => {
  ...
  if (!text) throw `🤖 ميزة الذكاء الاصطناعي \n\n▢ أمثلة :\n✪ ${usedPrefix + command} رشحلي انمي\n✪ ${usedPrefix + command} اعطيني سلسلة نتفلكس\n✪ ${usedPrefix + command} اعطيني نكتة`;
  try {
        conn.sendPresenceUpdate('composing', m.chat);
        //let sistema1 = await fetch(`https://raw.githubusercontent.com/Skidy89/chat-gpt-jailbreak/main/Text.txt`).then(v => v.text());
        let sistema1 = `سوف تعمل بمثابة WhatsApp Bot الذي أنشأه سوكونا و فينوم .`;
      ...
      try {
        conn.sendPresenceUpdate('composing', m.chat);
        const akuariapi2 = await fetch(`https://aemt.me/openai?text=${text}`);
      ...
      try {
        conn.sendPresenceUpdate('composing', m.chat);
        const akuariapi1 = await fetch(`https://api.lolhuman.xyz/api/openai-turbo?apikey=${lolkeysapi}&text=${text}`);
...
handler.command = /^(gpt|بوت|ميجو)$/i;
